...
const DetailOrder = () => {
  ...
  const getProducts = async (orderItems) => {
    const productPromises = orderItems
        .filter(item => item.productName) // Filter out items without a product name
        .map(async (item) => {
    ...
    return products.filter(product => product !== null); // Filter out null values
  };
                                ...
                                orderItems.map((item, i) => {
                                  ...
                                  return (
                                    <tr className="line-order odd" key={i}>
                                      ...
                                      <td className="money text-center">{(item.sellPrice)}</td>
